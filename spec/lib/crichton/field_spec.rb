...
module Crichton
  describe Field do
    ...
    describe '.new' do
      ...
      describe '#value #default #description #type #data_type' do
        it 'each of those methods match the hash' do
          semantic_elements_present =  %w(value default description type data_type).all? do |key|
            subject.send(key.to_sym) == field_hash.first[1][key.to_sym]
          end
      ...
      describe '#options' do
        it 'returns an options object' do
          ...
          subject.options.should be_an_instance_of(Crichton::Options)
        end
      ...
      describe '#validators' do
        it 'returns a list of hashes whose key is eqaul to the validator type' do
          validators = [{max_length: 8}, :required]
          @field_hash[:total_count][:validators] = validators
          hash_keys = validators.map do |hs| 
            hs.instance_of?(Symbol) ? hs : hs.keys[0]
          end
          object_keys = subject.validators.map { |hash| hash.keys[0] }
        ...
        it 'returns a list of hashes whose value must be matched against' do
          validators = [{max_length: 8}, :required]
          @field_hash[:total_count][:validators] = validators
          hash_values = validators.map do |hs| 
            hs.instance_of?(Symbol) ? hs : hs[hs.keys[0]]
          end
