  ...
  describe Transition do
    ...
    describe '.new' do
      it 'returns a Crichton::Transition instance' do
        subject.should be_an_instance_of(Crichton::Transition)
      end
      ...
      describe '#parameters' do
        it 'returns a list of fileds representing the link parameters' do
          @representor_hash = @search_transition
          field = subject.parameters.first
          field.to_hash.should == {name: @search_transition[:search][:descriptors][:name]}
        end
      end
      ...
      describe '#attributes' do
        it 'returns a list of fields representing the link attributes' do
          ...
          field.to_hash.should == {status: @search_transition[:search][:descriptors][:status]}
        end
      end           
      ...
      describe '#meta_links' do
        it 'returns a list of Transitions' do
          @representor_hash = @search_transition
          links = subject.meta_links.all? { |item|
            item.instance_of?(Crichton::Transition)
          }
