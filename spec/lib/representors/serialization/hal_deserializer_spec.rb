...
module Representors
  module Serialization
    describe HalDeserializer do
      subject(:deserializer) { HalDeserializer.new(document) }
      ...
      it "initializes with a JSON document" do
        expect(HalDeserializer.new({}.to_json)).to be_instance_of(HalDeserializer)
      end
    
      it 'provides the media-type application/vnd.hal+json' do
        media_types = HalDeserializer.media_types
        expect(media_types).to include('application/hal+json', 'application/json')
      end
    
      it 'provides the media symbol :hal' do
        expect(HalDeserializer.media_symbols).to include(:hal)
      end
